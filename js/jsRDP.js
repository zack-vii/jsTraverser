...
var simplifyPath = function( points, tolerance ) {
    ...
    var getTolerance = function(points) {
	var max = points[0].y;
	var min = points[0].y;
    ...
    var computedTolerance = (tolerance==null)?getTolerance(points):tolerance;
    var arr = douglasPeucker( points, computedTolerance);
    // always have to push the very last point on so it doesn't get left off
    arr.push( points[points.length - 1 ] );

    console.log("simplifyPath: " + points.length + 
		" -> " + arr.length + " (" +
		((tolerance==null)?"AUTO":"") + "e: " + computedTolerance.toFixed(4) + ")");
    return arr;
};
